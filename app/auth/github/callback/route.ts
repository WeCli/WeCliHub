...
export async function GET(request: NextRequest) {
  if (!GITHUB_CLIENT_ID || !GITHUB_CLIENT_SECRET) {
    return new NextResponse("<h2>GitHub OAuth not configured.</h2><a href='/'>Back to Teamclaw Hub</a>", {
      status: 500,
  ...
  if (oauthError) {
    return new NextResponse(`<h2>Authorization failed: ${oauthError}</h2><a href='/'>Back to Teamclaw Hub</a>`, {
      status: 400,
      headers: { "content-type": "text/html; charset=utf-8" }
    });
  }
  ...
  if (!code) {
    return new NextResponse("<h2>Authorization failed: no code provided.</h2><a href='/'>Back to Teamclaw Hub</a>", {
      status: 400,
      headers: { "content-type": "text/html; charset=utf-8" }
    });
  }
  if (!verifyOauthState(state)) {
    return new NextResponse("<h2>Authorization failed: invalid oauth state.</h2><a href='/'>Back to Teamclaw Hub</a>", {
      status: 400,
      headers: { "content-type": "text/html; charset=utf-8" }
    });
  }
